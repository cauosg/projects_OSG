...
tic
for ccount = 1:16
disp(strcat('step:',num2str(ccount)))
train_features = [];
test_features = [];
CS = ccount*2;
for i=1:strain
% disp(strcat('hognum:',num2str(i)))
[feature, ~]=extractHOGFeatures(train_images(:,:,i), 'CellSize',[CS CS],'BlockOverlap',[0,0]);
train_features = [train_features;feature];
end

for i=1:stest
[feature, ~]=extractHOGFeatures(test_images(:,:,i), 'CellSize',[CS CS],'BlockOverlap',[0,0]);
test_features = [test_features;feature];
end

tempknn = fitcknn(train_features,train_class);
tempecoc = fitcecoc(train_features,train_class);
test_rst = tempecoc.predict(test_features);
test_rst_k = tempknn.predict(test_features);

[~, accu1] = getaccuracy(test_rst,test_class);
accu_vec1(ccount) = accu1;

[~, accu2] = getaccuracy(test_rst_k,test_class);
accu_vec2(ccount) = accu2;
end
