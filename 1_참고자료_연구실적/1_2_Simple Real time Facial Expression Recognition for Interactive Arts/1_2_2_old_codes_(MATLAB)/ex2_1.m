...
for i = 1:emotion_nums
    ...
    [trainMats(:,:,i),temp_train] = HOG(CS, char(train_pname));
    Train_Class(nums_train : nums_train + temp_train - 1) = i;
    
    test_pname = strcat(basepath, '\', datasets(datanum), '\Test\', emotions(i))
    [testMats(:,:,i),temp_test] = HOG(CS, char(test_pname));
    Test_Class(nums_test : nums_test + temp_test - 1) = i;
    
    nums_train = nums_train + temp_train;
    nums_test = nums_test + temp_test;
end
