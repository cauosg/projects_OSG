...
float c_star1[3] = { 178,157,144 };
//float c_star1[3] = { 167,128,116 };
float c_star2[3] = { 165,81,69 };
//float c_star2[3] = { 121,65,47 };
float c_star3[3] = { 60, 39, 29 };
...
void main()
{
	...
	while (1) {
		...
		Mat sal1, sal2, sal0, sim, smap, cs_mat;
		vector<Mat> bgr, opcs;
		//////////////////////

		//

		bgr.push_back(one * (c_star1[2] / 256));
		bgr.push_back(one * (c_star1[1] / 256));
		bgr.push_back(one * (c_star1[0] / 256));


		merge(bgr, cs_mat);

		opcs = bgr_to_opponent(cs_mat);

		//여기 구간은 밖으로 빼도 된다!

		float w0 = 2;//
		float w1 = 20;//sensitivity, 0~1 : 4
		float w2 = 0.14285 / 3;//
		...
		sim = sal1.mul(sal2).mul(sal0);
		//threshold(sim, sim, mean(sim).val[0], 1.0, THRESH_BINARY);
		sim.copyTo(smap);
		regshow("seg1", sim, procsize);
		bgr.clear();
		bgr.push_back(one * (c_star2[2] / 256));
		bgr.push_back(one * (c_star2[1] / 256));
		bgr.push_back(one * (c_star2[0] / 256));
		...
		sim = sal1.mul(sal2).mul(sal0);
		//threshold(sim, sim, mean(sim).val[0], 1.0, THRESH_BINARY);
		regshow("seg2", sim, procsize);
		smap = smap + sim;
		bgr.clear();
		bgr.push_back(one * (c_star3[2] / 256));
		bgr.push_back(one * (c_star3[1] / 256));
		bgr.push_back(one * (c_star3[0] / 256));
		...
		//threshold(sim, sim, mean(sim).val[0], 1.0, THRESH_BINARY);
		bgr.clear();
		regshow("seg3", sal0, procsize);
		regshow("sim", sim, procsize);
		smap = smap + sim;
		//////////////////////
		Mat n_eigen, n_bgr;

		GaussianBlur(smap, smap, Size(3, 3), 0);

		threshold(smap, n_eigen, mean(smap).val[0], 1.0, THRESH_BINARY);
