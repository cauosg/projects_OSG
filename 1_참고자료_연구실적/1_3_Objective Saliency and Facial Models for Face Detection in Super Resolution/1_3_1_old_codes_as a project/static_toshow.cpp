...
void main()
{
	...
	threshold(sim, t1, mean(sim).val[0] * (1 + sigma), 1.0, THRESH_BINARY);
	threshold(sim, t2, mean(sim).val[0] * (1 - sigma), 1.0, THRESH_BINARY);
	//regshow("threshold1", t1, procsize);
	//regshow("threshold2", t2, procsize);

	//regshow("sum of threshold", t1, procsize);
	n_eigen = t1 + t2;
	threshold(n_eigen, n_eigen, 0.00000001, 1.0, THRESH_BINARY);
	//double adg = mean(sim).val[0];
	cvtColor(n_eigen, n_eigen, CV_GRAY2BGR);
	...
	Mat dst = n_eigen.mul(p720);// *0.00392;
	...
	double scale)
{
	...
	cascade.detectMultiScale(gray, faces, scale, 3, 0, Size(80,45));
