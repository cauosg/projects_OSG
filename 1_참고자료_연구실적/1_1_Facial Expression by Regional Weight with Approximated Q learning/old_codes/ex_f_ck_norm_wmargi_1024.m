...
min_csize = 8;
now_emo = 2;
imgtoshow = imresize(imread('S052_004_00000033.png'),[res_proc, res_proc]);
nums_emotion = 7;
...
for iter = 1:max_iter
    ...
    for ind_grid=1:num_grids
        ...
        for img_ind = 1:nums_train
            timg = cell2mat(tot_imgset(img_ind));
            temp_features = [];
            for ci = 1:num_ci
    ...
    for di = 1:grids_to_draw
        timg= insertObjectAnnotation(imgtoshow,'rectangle',out_grids(:,:,di) + [0,0,1,1],'','Color','green');
    end
    ...
    acctory(iter) = t_accu;
    %save(strcat('train_rst\classifier_',num2str(iter),'.mat'),'rst_svm');
    save(strcat('train_rst\features_',num2str(iter),'.mat'),'rst_mat');
