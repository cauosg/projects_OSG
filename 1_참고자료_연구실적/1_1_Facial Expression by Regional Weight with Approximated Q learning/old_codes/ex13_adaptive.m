...
subplot(2,2,datanum);
C = confusionmat(test_class,test_rst);
...
svm = fitcecoc(total_train_mats, total_train_class,'Options',options);

cvval = crossval(svm,'Options',options);
loss = kfoldLoss(cvval)
%% testing
total_train_rst = kfoldPredict(cvval,'Options',options);

[errorsum_test t_accu] = getaccuracy(total_train_rst,total_train_class);
%% show
figure; 
set(gcf, 'Position',  [500, 10, 700, 700]);

Ct= confusionmat(total_train_class, total_train_rst);
