...
for ind_iter = 1:max_iter
    ...
    max_eval = 0;
    ind_omax = 0;
    for ind_org = 1:num_grids
        grid_temp = origin_grids(ind_org,:);
        max_temp = max(max_eval,max(max(mat_eval(grid_temp(1):min(grid_temp(1) + grid_temp(3),res_proc),...
            grid_temp(2):min(grid_temp(2) + grid_temp(4),res_proc)))));
        if max_eval < max_temp
            max_eval = max_temp;
            ind_omax = ind_org;
        end
    end
    ...
    [~,input_grids] = step_grid(origin_grids(ind_omax,:));
    out_grids = grid_extract(origin_grids,input_grids,ind_omax);
    ...
    origin_features = rst_mat(:,:,ind_omax);
