...
for ind_iter = 1:50
    ...
    for ind_grid=1:num_grids
        [proc_len,nex_grids] = divide_grid(origin_grids(ind_grid,:));
        ...
        %tot_features = feature_cat(origin_features,tot_features,ind_grid);
         %% train
        totd_class = train_class(:,1:ind_iter);
        
        otd_class = totd_class(:,1);
        if ind_iter>1
            for ind_class = 1:ind_iter-1
                otd_class = [otd_class;totd_class(:,ind_class + 1)];
            end    
        end
    ...
    saveas(gcf,strcat('train_rst\grids_epoch',num2str(ind_iter),'.png'));
    save(strcat('train_rst\rewards_epoch',num2str(ind_iter),'.mat'),'rewards');
    %% show confumat
    ...
    saveas(gcf,strcat('train_rst\confusion_epoch',num2str(ind_iter),'.png'));    
end