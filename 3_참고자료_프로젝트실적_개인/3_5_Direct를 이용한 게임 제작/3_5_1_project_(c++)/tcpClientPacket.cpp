...
#include "Packet.h"
//#include "fireTestSol.cpp"

SOCKET socketConnect = INVALID_SOCKET;
extern int sendshape;
extern int gamemode;
extern int recvmode;
	...
	while (1){
		...
		while (receivedPacketSize > 0)	// parsing Packet Length
		{
			...
			if (receivedPacketSize >= packetlength)
			{
				...
				sscanf(recvBuffer, "%d %d %d %d %d %d %d %d %d",
					&Ginfo.posX2p, &Ginfo.posY2p, &heroobj.scene, &Ginfo.hp[1], &Ginfo.mp[1], &recvmode, &Ginfo.player2, &Ginfo.score2p, &Ginfo.velocity2p, &Ginfo.p2scene);
...
void comsend(){
	...
	sprintf(senddata, "%d %d %d %d %d %d %d %d %d",
		heroobj.posX, heroobj.posY, heroobj.scene,
		Ginfo.hp[0], Ginfo.mp[0], gamemode, iq, Ginfo.score, Ginfo.p1scene);
	sendpacket.clear();
	sendpacket.id(1001);
	sendpacket.writeData(senddata, strlen(senddata) + 1);

	sendbytes = ::send(socketConnect, sendpacket.getPacketBuffer(), sendpacket.getPacketSize(), 0);
