...
void pos_send(HANDLE &hDevice, Point2f pos, int zdiff) {
	static int fcount = 0;
	...
	//int ydiff = fcount *  nx / fps + 90;
	//int xdiff = fcount * ny / fps + 90;


	
	int ydiff = (320 - pos.x)*w_rate + 90;
	int xdiff = (180 - pos.y)*w_rate + 153;

	char sdata[9];
	sdata[0] = (ydiff / 100) + '0';
	sdata[1] = ((ydiff % 100) / 10) + '0';
	sdata[2] = (ydiff % 10) + '0';
	sdata[3] = (xdiff / 100) + '0';
	sdata[4] = ((xdiff % 100) / 10) + '0';
	sdata[5] = (xdiff % 10) + '0';
	sdata[6] = (zdiff / 100) + '0';
	sdata[7] = ((zdiff % 100) / 10) + '0';
	sdata[8] = (zdiff % 10) + '0';

	DWORD btsIO;
	if (WriteFile(hDevice, sdata, 9, &btsIO, NULL)) {
		for (int i = 0; i < 3; i++) {
			cout << sdata[i];
		}
		cout << endl;
...
Point2f draw_sds(Mat &f, Point3f a_sd, int iters) {
...
*/
	...
	//CSerialComm handle;
	//if (!handle.connect("COM5"))
	//	cout << "connection error" << endl;


	string address = "rtsp://192.168.42.1/live";
	//string address = "rtsp://192.168.42.1:554/live";

	VideoCapture cap(0);//9,10,11,15
	cap.set(CV_CAP_PROP_FRAME_WIDTH, 640);
	cap.set(CV_CAP_PROP_FRAME_HEIGHT, 360);
	if (!cap.isOpened())
	{
		cout << "cam is not opened";
		char inputt;
		cin >> inputt;
	}
	int padsize = 20;
	...
	while (waitKey(10) != 27) {
		//if (set_fps(30)) 
		{
			...
			if (saliency(f_src, bp10, sd)) {
				Point2f salpos = draw_sds(out_sal, sd, iter_nums);
				pos_send(hDevice,salpos,90);
			}

			saliency(f_src, bp10, sd);
			//resize(out, out, Size(cols, rows));
			resize(out_props, out_props, Size(800, 480));
			resize(out_van, out_van, Size(800, 480));
			resize(out_sal, out_sal, Size(800, 480));
